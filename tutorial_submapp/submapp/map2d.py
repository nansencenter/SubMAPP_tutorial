...
class Map2d:
    ...
    - :ref:`rmse_with_respect_to_depth <submapp.map2d.Map2d.rmse_with_respect_to_depth>`,
      pandas.DataFrame that contains RMSE for each variables for each depth level.
    ...
    def rmse_with_respect_to_depth(self, depth: np.ndarray):
        """ Computes the RMSE of the map for each variables
        at each depth levels.
        This method uses the `values` and `true_values` 
        attributes of the map. It means that it will
        compute the RMSE according to the data passed in
        the last call of the `map` method. 
        
        :param depth: Vector of depth levels
        :type depth: np.ndarray
        :return: Dataframe of the RMSE for each variable
        :rtype: pd.DataFrame
        """

        variables = self.som.variables
        n_depth_levels = int(self.som.p / len(variables))
        RMSE_df = DataFrame(columns=['depth']+variables)
        RMSE_df['depth'] = depth
        for i in range(len(variables)):
            RMSE_variable_i = []
            values_to_consider = self.values[:,i*n_depth_levels:(i+1)*n_depth_levels]
            true_values_to_consider = self.true_values[:,i*n_depth_levels:(i+1)*n_depth_levels]
            for depth_level in range(n_depth_levels):
                values_mapped = values_to_consider[:,depth_level]
                true_values = true_values_to_consider[:,depth_level]
                RMSE_variable_i.append(np.sqrt(np.nanmean((values_mapped-true_values)**2)))
            RMSE_df[variables[i]] = RMSE_variable_i
        return RMSE_df
