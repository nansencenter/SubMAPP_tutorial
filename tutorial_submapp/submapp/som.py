...
class Som:
    ...
    .. csv-table::
       ...
       :ref:`print_weights_pca_axis <submapp.som.Som.print_weights_pca_axis>`,\
    ...
    def __init__(
        ...
        self.__n_variables = len(variables)
    ...
    def __init_graph(
        ...
        with self.__graph.as_default():
            ...
            vect_input = tf.compat.v1.placeholder(
                tf.float32, (1, self.__p), name="Input")
            # distance_to_bmu[k] = distance_matrix[bmu, k]
            distance_to_bmu = tf.compat.v1.placeholder(
                tf.float32, (self.nb_class, 1), name="Distance_to_BMU")
            curr_it = tf.compat.v1.placeholder(tf.float32, name="Current_Iteration")
            T_train = tf.compat.v1.placeholder(tf.float32, name="T_train")

            # Find the BMU (class whose referent vector minimizes the
            # euclidean distance to the input)
            tf.argmin(
                input=tf.sqrt(tf.reduce_sum(input_tensor=(weights - vect_input)**2, axis=1)),
                name="BMU_op")
    ...
    def sample_weights_initialization(
        ...
        data_train_available = remove_nan(data_train,False)
    ...
    def random_weights_initialization(
        ...
        :param distribution: 
            ...
            None, in this case the distribution is uniform. "gaussian_fix" is in the case of a cross-validation,
            in this way each step of the cross-validation is initialized with the same Gaussian
        ...
        """
        if distribution == "gaussian_fix":
            sp.random.seed(16)
            self.__weights = sp.random.normal(
    ...
    def train(
        ...
        sess = tf.compat.v1.Session(graph=self.__graph)
        ...
        with self.__graph.as_default() as g:
            ...
            for t in shuffled_time:
                ...
                if(np.count_nonzero(np.isnan(v_i))==0):
                    # Find the BMU
    ...
    def map(
        ...
        weights = tf.compat.v1.placeholder(tf.float32, (self.nb_class, self.__p))
        vect_input = tf.compat.v1.placeholder(tf.float32, (1, self.__p))
        difference = weights - vect_input
        difference = tf.compat.v1.where(tf.math.is_nan(difference), 
                                tf.zeros_like(difference), 
                                difference)
        bmu_loc_op = tf.argmin(input=tf.sqrt(tf.reduce_sum(input_tensor=difference**2, axis=1)))
        ...
        for v_i in data_map:
            ...
            # affect default class to bmu_loc if one of the vector is missing
    ...
    def print_weights_pca_axis(self, data, is_standardized=False):
        """Print the weights of the map on first two axis of the PCA

        This is a visual way to check out how the map is actually 
        deploying on the data. Note that the interpretation really 
        depends on the quality of the projection on first two axis
        of the PCA. 
        
        :param data: 
            Matrix whose element are the data stardized or not
            on which we will apply the PCA. The matrix should
            not distinguish the years, you can use ``np.concatenate``
            to get rid of the years.
        :type data: np.ndarray[float, float], shape = (_,p)
        :param is_standardized:
            Set to true if the data passed in argument is
            standardized, else false. Default to false.
            If it is set to false, then the data is standardized.
        :rtype: None
        """
    ...
    def destandardize(self, data_norm: np.ndarray) -> np.ndarray :
        ...
        """
        destandardized_array = np.copy(data_norm)
        if self.p != len(self.__stdev):
            n_variables = len(self.__stdev)
            n_depth_levels = int(self.p/n_variables)
            # We iterate over the number of variables
            
            for i in range(n_variables):
                mean = self.__mean[i]
                stdev = self.__stdev[i]
                destandardized_array[:,i*n_depth_levels:(i+1)*n_depth_levels] = data_norm[:,i*n_depth_levels:(i+1)*n_depth_levels]*stdev + mean
        else:
            destandardized_array = data_norm * self.__stdev + self.__mean
    ...
    def variables(self) -> list:
        """ List of the variables used for the training
